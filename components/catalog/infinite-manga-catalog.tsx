...
interface InfiniteMangaCatalogProps {
  initialItems: MangaUpdateItem[];
}

const CATEGORIES = [
  { label: "All Top Manga", value: "manga" },
  { label: "Korean Manhwa Webtoons", value: "manhwa" },
  { label: "Most Popular", value: "bypopularity" },
  { label: "Top Rated Favorites", value: "favorite" },
] as const;

export function InfiniteMangaCatalog({ initialItems }: InfiniteMangaCatalogProps) {
  const [items, setItems] = useState<MangaUpdateItem[]>(initialItems);
  const [selectedCategory, setSelectedCategory] = useState<string>("manga");
  ...
  const handleCategoryChange = async (cat: string) => {
    ...
    try {
      const res = await fetch(`/api/mal/top-manga?ranking_type=${cat}&limit=24&offset=0`);
  ...
  const loadMore = async () => {
    ...
    try {
      const res = await fetch(
        `/api/mal/top-manga?ranking_type=${selectedCategory}&limit=24&offset=${offset}`
      );
      const data = await res.json();
        ...
        {hasMore ? (
          ...
          >
            {loading ? (
              ...
              <>
                <span>Load More Manga & Manhwa (+24)</span>
